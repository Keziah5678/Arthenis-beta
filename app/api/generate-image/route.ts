...
export async function POST(request: Request) {
  ...
  if (!context || !entity?.name || !entity?.description) return NextResponse.json({ error: "Invalid visual generation request." }, { status: 400 });
  ...
  if (image?.b64_json) return NextResponse.json({ imageData: "data:image/png;base64," + image.b64_json, visualSpec });
