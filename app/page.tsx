...
export default function Home(){
 ...
 async function createItem(){
  ...
  else if(kind==="Personnage"){table="characters";row={world_id:world.id,name:finalName,biography:text,ai_can_evolve:true};}
 ...
 {step===3&&<section className="createCard"><p className="panelTag">NAISSANCE</p><h1>{title} est prêt à naître.</h1><p>Arthenis créera une mémoire, des règles et une base persistante pour ton monde.</p><div className="heroActions"><button onClick={()=>setStep(2)}>← Retour</button><button className="primary" disabled={busy} onClick={createWorld}>{busy?"Création…":"Découvrir le monde →"}</button></div>{notice&&<p>{notice}</p>}</section>}{authOpen&&<AuthModal authMode={authMode} email={email} password={password} busy={busy} notice={notice} onClose={()=>setAuthOpen(false)} onEmailChange={setEmail} onPasswordChange={setPassword} onAuthenticate={authenticate} onResetPassword={resetPassword} onToggleMode={()=>setAuthMode(v=>v==="signin"?"signup":"signin")}/>}</main>;
 ...
 {authOpen&&<AuthModal authMode={authMode} email={email} password={password} busy={busy} notice={notice} onClose={()=>setAuthOpen(false)} onEmailChange={setEmail} onPasswordChange={setPassword} onAuthenticate={authenticate} onResetPassword={resetPassword} onToggleMode={()=>setAuthMode(v=>v==="signin"?"signup":"signin")}/>} 
 </main>;
}